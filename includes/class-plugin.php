		...
		private function scan_posts( array $post_ids, array &$state ): void {
			...
			foreach ( $posts as $post ) {
				...
				$source_id            = (int) $post->ID;
					...
					$editable_href_map    = $should_count_internal
						? $this->extract_editable_href_map( (string) $post->post_content )
						: array();
					...
					foreach ( $links as $link ) {
						...
						foreach ( $target_ids as $target_id ) {
							...
							$is_editable_href = isset( $editable_href_map[ $this->normalize_match_url( $href ) ] );
							$this->record_incoming_detail( $state, $target_id, $source_id, $anchor_text, $href, $is_editable_href );
			 ...
			 * @param bool                $editable    Whether this href exists in editable post content.
			 */
			private function record_incoming_detail( array &$state, int $target_id, int $source_id, string $anchor_text, string $href, bool $editable ): void {
				...
				$was_editable = ! empty( $state['incoming_details'][ $target_id ][ $source_id ]['editable'] );

				// Prefer an editable href so the remove/replace action can match stored content.
				if (
					'' !== trim( $href )
					&& (
						'' === (string) ( $state['incoming_details'][ $target_id ][ $source_id ]['raw_url'] ?? '' )
				...
				if ( $editable ) {
					$state['incoming_details'][ $target_id ][ $source_id ]['editable'] = true;
				}
			...
			/**
			 * Build a normalized href lookup from raw, editable post content.
			 *
			 * Links produced only by rendered blocks or shortcodes are counted, but
			 * cannot be modified by the post-content editor.
			 *
			 * @param string $content Raw post content.
			 * @return array<string,bool>
			 */
			private function extract_editable_href_map( string $content ): array {
				$hrefs = array();

				foreach ( $this->extract_links( $content ) as $link ) {
					$href = $this->normalize_match_url( (string) ( $link['href'] ?? '' ) );
...
/**
			...
			private function collect_link_health( array &$state, WP_Post $post, string $source_url, string $href, string $anchor_text, array $target_ids, string $home_host, bool $home_is_https ): void {
				...
				if ( 0 === strpos( $trimmed_href, '//' ) ) {
					$home_scheme  = strtolower( (string) ( wp_parse_url( home_url( '/' ), PHP_URL_SCHEME ) ?: 'https' ) );
					$trimmed_href = $home_scheme . ':' . $trimmed_href;
				}
		...
		private function build_incoming_detail_rows( array $state, int $target_id ): array {
			...
			foreach ( $sources as $source_id => $info ) {
				...
				$rows[] = array(
					...
					'editable' => array_key_exists( 'editable', (array) $info ) ? ! empty( $info['editable'] ) : true,
				);
			}
